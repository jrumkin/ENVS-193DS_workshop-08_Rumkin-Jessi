...
drought_exp_clean <- drought_exp |>  # starting with the drought_exp object
  ...
  # making a column of full name for drought treatment
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  )) |> 
  # makes the species name a factor instead of characters (factor with 7 levels when checked in the console how??)
  mutate(species_name = as_factor(species_name),
         species_name = fct_relevel(species_name,
                                    ...
                                    "Grindelia camporum")) |> 
  # makes the water treatment a factor instead of characters (run str(drought_exp_clean) in console)
  mutate(water_treatment = as_factor(water_treatment),
         water_treatment = fct_relevel(water_treatment,
                                       "Drought stressed",
                                       "Well watered")) |> 
  # select on these 4 columns for species, treatment, sla, and total g
...
ggplot(data = drought_exp_clean,
  ...
  stat_summary(geom = "point",
               fun = mean,
               size = 4) +
  # making sure that axis labels wrap nicely for scale_x the label wrap is at 10
...
model1 <- lm(
  total_g ~ sla + water_treatment + species_name, #formula (this is the saturated model)
...
#Yes you have to write this out by hand. We are critically choosing each model and what the variables are for each one. 
...
par(mfrow = c(2,2)) #displays the plots in a 2 by 2
plot(model1)
plot(model2)
plot(model3)
plot(model4) #this one looks a little different because its only categorical data, that's fine. Many of these will be like that
plot(model5)
plot(model6)
plot(model7)
...
summary(model4)
#reference level for water_treatment: drought stressed (they are comping well watered TO drought stressed)
#reference level for species_name: lotus scoparius (its the only one that doesn't show up in the summary!)
#the first level that you set, that will always be the reference. That was set above when set the reference.
...
#the adjusted r-squared is .7257 so this tells us the 73%
...
Across water treatments, all species have different (and higher) masses than _Lotus scoparius_ expect _Penstemon centranthifolius_.
